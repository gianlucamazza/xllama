...
| `0002-ggml-clang-cl-interference-size.patch` | Exclude **clang-cl** from `__cpp_lib_hardware_interference_size` in `ggml/src/ggml-cpu/ops.h`. The xwin/UWP CRT advertises the feature macro but does not provide `std::hardware_destructive_interference_size`, so clang-cl fails to compile ggml; real MSVC and host libstdc++/libc++ are unaffected and keep the fast path. Needed by `uwp-crossbuild` only, and a no-op for the CI MSVC build, which is why it is a separate patch rather than folded into 0001.                                                                                                                                                                                                    |
