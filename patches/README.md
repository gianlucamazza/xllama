...
| File                                        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0001-uwp-appcontainer-guards.patch`        | Guard Win32 desktop-only APIs across **4 files**. Guards use **`XLLAMA_UWP`** (set on `ggml-uwp.vcxproj` / `xllama.vcxproj`) **and/or** `WINAPI_FAMILY_PARTITION(DESKTOP)` so **uwp-crossbuild** (which does not define `WINAPI_FAMILY=APP` — see its gotcha 22) still stubs registry CPU-name (`ggml-cpu.cpp`), `SetThreadAffinityMask` / power throttling (`ggml-cpu.c`), packaged `LoadPackagedLibrary` (`ggml-backend-dl.cpp`), and desktop mmap/mlock (`llama-mmap.cpp`). Without `XLLAMA_UWP`, a crossbuild PE imported `RegOpenKeyExA` + `SetThreadAffinityMask` and failed Xbox activation `0x8027025b`. Always run `./scripts/apply-uwp-patches.sh` before a Linux UWP build. |
