...
void ChatHistory::LoadIndex() {
    ...
    std::string json;
    char buf[4096];
    while (size_t n = fread(buf, 1, sizeof(buf), f)) json.append(buf, n);
    fclose(f);
    ...
    while (pos < json.size()) {
        ...
        for (int k = 0; k < 8 && pos < json.size(); ++k) {
            ...
            if (!seek_char(json, pos, ':')) break;
            ++pos;
            std::string val = json_read_value(json, pos);
            if (key == "id")            meta.id = val;
            else if (key == "title")    meta.title = val;
            else if (key == "last_modified") meta.last_modified = std::stoll(val.empty() ? "0" : val);
            else if (key == "n_messages")    meta.n_messages   = std::stoi(val.empty() ? "0" : val);
        }
        if (!meta.id.empty())
...
void ChatHistory::SaveIndex() {
    ...
    for (size_t i = 0; i < m_index.size(); ++i) {
        ...
        fprintf(f, "  {\"id\":\"%s\",\"title\":\"%s\",\"last_modified\":%lld,\"n_messages\":%d}%s\n",
                json_escape(m.id).c_str(),
                json_escape(m.title).c_str(),
                static_cast<long long>(m.last_modified),
                m.n_messages,
                (i + 1 < m_index.size()) ? "," : "");
    }
    fputs("]\n", f);
...
void ChatHistory::Save(const Conversation& conv) {
    ...
    for (size_t i = 0; i < conv.messages.size(); ++i) {
        ...
        fprintf(f, "  {\"role\":\"%s\",\"content\":\"%s\",\"ts\":%lld,\"partial\":%s}%s\n",
                role_str,
                json_escape(msg.content).c_str(),
                static_cast<long long>(msg.ts_unix),
                msg.partial ? "true" : "false",
...
Conversation ChatHistory::Load(const std::string& id) const {
    ...
    if (msgs_pos == std::string::npos) return conv;
    ...
    // Parse id and title first
    {
        ...
        while (p2 < msgs_pos) {
            skip_ws(json, p2);
            if (p2 >= msgs_pos) break;
            if (json[p2] == ',') { ++p2; continue; }
            if (json[p2] != '"') { ++p2; continue; }
            std::string key = json_read_key(json, p2);
            if (!seek_char(json, p2, ':')) break;
            ++p2;
            std::string val = json_read_value(json, p2);
            if (key == "id")    conv.id = val;
            if (key == "title") conv.title = val;
        }
    }

    // Parse messages array
    pos = msgs_pos + 10; // skip "messages"
    if (!seek_char(json, pos, '[')) return conv;
    ++pos;
    while (pos < json.size()) {
        skip_ws(json, pos);
        if (pos < json.size() && json[pos] == ']') break;
        ...
        for (int k = 0; k < 10 && pos < json.size(); ++k) {
            ...
            std::string val = json_read_value(json, pos);
            if (key == "role") {
                if (val == "system")    msg.role = MessageRole::System;
                else if (val == "user") msg.role = MessageRole::User;
                else                    msg.role = MessageRole::Assistant;
            } else if (key == "content") {
                msg.content = val;
            } else if (key == "ts") {
