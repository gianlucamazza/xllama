...
InferenceResult run_inference(const InferenceParams& params) {
    ...
    int32_t n_tokens = llama_tokenize(
        vocab,
        params.prompt.c_str(), static_cast<int32_t>(params.prompt.size()),
        nullptr, 0,
    ...
    while (n_generated < params.n_predict) {
        ...
        if (len > 0) {
            buf[len] = '\0';
            res.output_text += buf;
            if (params.on_token) params.on_token(std::string(buf, static_cast<size_t>(len)));
#ifndef XLLAMA_UWP
        ...
        if (llama_decode(ctx.get(), next) != 0) break;
        ++n_generated;
    }
    ...
    snprintf(log_buf, sizeof(log_buf),
             ...
             res.n_p_eval > 0 && res.t_p_eval_ms > 0
                 ? static_cast<double>(res.n_p_eval) / (res.t_p_eval_ms / 1000.0) : 0.0,
             res.n_eval > 0 && res.t_eval_ms > 0
                 ? static_cast<double>(res.n_eval) / (res.t_eval_ms / 1000.0) : 0.0,
             res.peak_ws_mb);
    log_output(log_buf);
