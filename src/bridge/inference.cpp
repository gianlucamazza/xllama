...
#ifdef XLLAMA_UWP
        // --- Win32 access probes: identify why weakly_canonical fails ---
        {
            int wsz = MultiByteToWideChar(CP_UTF8, 0, model_dir.c_str(), -1, nullptr, 0);
            std::wstring mdw;
            if (wsz > 0) {
                mdw.resize(static_cast<size_t>(wsz));
                MultiByteToWideChar(CP_UTF8, 0, model_dir.c_str(), -1, mdw.data(), wsz);
                if (!mdw.empty() && mdw.back() == L'\0') mdw.pop_back();
            }
            if (!mdw.empty()) {
                auto cf2 = [](const wchar_t* path, DWORD acc, const char* tag) {
                    CREATEFILE2_EXTENDED_PARAMETERS ep{};
                    ep.dwSize = sizeof(ep);
                    ep.dwFileFlags = FILE_FLAG_BACKUP_SEMANTICS;
                    HANDLE h = CreateFile2(path, acc,
                        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                        OPEN_EXISTING, &ep);
                    char b[256];
                    if (h == INVALID_HANDLE_VALUE)
                        snprintf(b, sizeof(b), "[xllama][probe] CF2 %-20s acc=%08lX FAIL err=%lu\n",
                                 tag, acc, GetLastError());
                    else { snprintf(b, sizeof(b), "[xllama][probe] CF2 %-20s acc=%08lX OK\n", tag, acc); CloseHandle(h); }
                    log_output(b);
                };
                // Probe 1-2: GetFileAttributesW
                auto gfa = [](const wchar_t* path, const char* tag) {
                    DWORD attr = GetFileAttributesW(path);
                    char b[256];
                    if (attr == INVALID_FILE_ATTRIBUTES)
                        snprintf(b, sizeof(b), "[xllama][probe] GFA %-20s FAIL err=%lu\n", tag, GetLastError());
                    else
                        snprintf(b, sizeof(b), "[xllama][probe] GFA %-20s OK attr=%08lX\n", tag, attr);
                    log_output(b);
                };
                gfa(mdw.c_str(), "model_dir");
                gfa((mdw + L"\\genai_config.json").c_str(), "genai_config.json");
                ...
                if (p1 != std::wstring::npos) {
                    std::wstring par = mdw.substr(0, p1);
                    cf2(par.c_str(), GENERIC_READ, "models/");
                    auto p2 = par.rfind(L'\\');
                    if (p2 != std::wstring::npos)
                        cf2(par.substr(0, p2).c_str(), GENERIC_READ, "LocalState/");
                }
                // Probe 6: chdir + CreateFile2(".")
                wchar_t old_cwd[1024]{};
                GetCurrentDirectoryW(1024, old_cwd);
                if (SetCurrentDirectoryW(mdw.c_str())) {
                    cf2(L".", GENERIC_READ, "dot(after_chdir)");
                    SetCurrentDirectoryW(old_cwd);
                } else {
                    char b[128]; snprintf(b, sizeof(b), "[xllama][probe] chdir FAIL err=%lu\n", GetLastError());
                    log_output(b);
                }
            }
        }
