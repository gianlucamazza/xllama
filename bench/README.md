...
| `results/phase16-gguf.csv`                         | 16    | llama.cpp CPU (Series S, t6)    | Phase 16 model-scouting T3 (2026-08-10, MSIX 1.5.4.887, `n_ctx` 2048, `n_predict` 96, 3 recorded `run_index` each): `lfm25-230m` **PASS** 119.77 tok/s @ 241 MB; `qwen35-2b` FAIL 19.25 @ 1421 MB; `maincoder-1b` FAIL 33.49 @ 843 MB. Also the evidence that the `weights × 1.12` peak factor **under-predicts** (−4% to −47%) — overhead is a ~95 MB floor plus an arch-dependent KV term. Cards and verdicts: `docs/phase16-model-scouting.md`                                                                                                                                                                                                                                                                                                                |
