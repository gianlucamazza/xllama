...
| WS-F | H16.6      | **open** — probe written and run; graph opens, no capture device was attached     |
...
| 2026-08-10 | WS-F           | Mic probe written and run on 1.5.4.897. The `microphone` capability installs; `AudioGraph` opens under AppContainer at 48 kHz stereo; `AccessDenied` did **not** fire. No headset was attached, so the device node returned `DeviceNotAvailable` — a fact about the room, not a verdict. WS-F stays **open**, one rerun with a microphone away from settling. A boolean probe would have scored this FAIL and closed the workstream on a test that never reached the microphone. | [uwp-constraints.md](uwp-constraints.md) §10d       |
